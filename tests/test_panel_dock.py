...
class PanelDockContractTests(unittest.TestCase):
    ...
    def test_pinned_ideas_stay_on_outline_not_widget_rail(self) -> None:
        ...
        self.assertIn("function syncHeaderNoticeStripLayout(", self.js)
        self.assertIn("function scheduleHeaderNoticeStripLayout(", self.js)
        self.assertIn("function setupHeaderNoticeStripLayout(", self.js)
        self.assertIn('workArea', self.js.split("function syncHeaderNoticeStripLayout(", 1)[1].split("function setupHeaderNoticeStripLayout(", 1)[0])
        self.assertIn("scheduleHeaderNoticeStripLayout();", self.js.split("function applyOutlineWidth(", 1)[1][:500])
        self.assertIn("scheduleHeaderNoticeStripLayout();", self.js.split("function applyAiPanelWidth(", 1)[1][:500])
    ...
    def test_manuscript_context_menu_fits_viewport(self) -> None:
        ...
        self.assertIn("function insertManuscriptDivider", self.js)
        self.assertIn("MANUSCRIPT_DIVIDER_LINES", self.js)
        self.assertIn("function readDividerAlign", self.js)
        self.assertIn("function writeDividerAlign", self.js)
        self.assertIn("function syncDividerOptionUi", self.js)
        self.assertIn("supertory.dividerAlign", self.js)
        self.assertIn("supertory.dividerKind", self.js)
        self.assertIn("function saveDividerPresetFromInput", self.js)
        self.assertIn("function renderDividerPresetPills", self.js)
        self.assertIn("supertory.dividerPresets", self.js)
        self.assertIn('id="dividerPresetRow"', self.html)
        self.assertIn('id="dividerPresetSaveBtn"', self.html)
        self.assertIn('data-i18n="index.구분선_추가"', self.html)
        self.assertIn("예: ※ 다음 장면 ※", self.html)
        self.assertIn('data-divider-kind="solid"', self.html)
        self.assertIn('data-divider-kind="dotted"', self.html)
        self.assertIn('data-divider-kind="stars"', self.html)
        self.assertIn('data-divider-kind="wave"', self.html)
        self.assertIn('data-divider-kind="middot"', self.html)
        self.assertIn('data-divider-kind="diamond"', self.html)
        self.assertIn('data-divider-kind="dash"', self.html)
        self.assertIn('data-divider-kind="blank"', self.html)
        self.assertIn('dataset.dividerKind = "preset"', self.js)
        self.assertIn('id="dividerOptionsToggle"', self.html)
        self.assertIn('id="dividerOptionsRow"', self.html)
        self.assertIn('id="dividerAlignRow"', self.html)
        self.assertIn('id="dividerCustomRow"', self.html)
        self.assertIn('id="dividerCustomInput"', self.html)
        self.assertIn('data-context-action="insert-divider"', self.html)
        self.assertIn('data-divider-align="center"', self.html)
        self.assertIn('data-divider-align="left"', self.html)
        self.assertIn('data-divider-align="right"', self.html)
        self.assertIn('.rich-editor .manuscript-scene-break[data-align="left"]', self.css)
        self.assertIn('.rich-editor .manuscript-scene-break[data-align="right"]', self.css)
        self.assertIn(".divider-custom-input", self.css)
        divider_toggle = self.html.split('id="dividerOptionsToggle"', 1)[1][:220]
        self.assertIn("data-ctx-full-only", divider_toggle)
        insert_divider = self.html.find('id="dividerOptionsToggle"')
        self.assertGreater(insert_divider, insert_table)
        ...
        for locale in self.locales.values():
            ...
            self.assertIn("index.구분선_삽입", locale)
            self.assertIn("index.구분선_실선", locale)
            self.assertIn("index.구분선_점선", locale)
            self.assertIn("index.구분선_별표", locale)
            self.assertIn("index.구분선_물결", locale)
            self.assertIn("index.구분선_가운뎃점", locale)
            self.assertIn("index.구분선_마름모", locale)
            self.assertIn("index.구분선_대시", locale)
            self.assertIn("index.구분선_공백", locale)
            self.assertIn("index.구분선_정렬", locale)
            self.assertIn("index.구분선_가운데", locale)
            self.assertIn("index.구분선_직접_입력", locale)
            self.assertIn("index.구분선_추가", locale)
    ...
    def test_titlebar_file_menu_and_project_library(self) -> None:
        ...
        self.assertNotIn('id="titlebarHelpButton"', header)
        self.assertIn('id="adminModeButton"', header)
        self.assertIn('id="ambientSoundButton"', header)
        self.assertIn('id="gitsiPopoverButton"', header)
        self.assertIn('id="uiThemeToggleButton"', header)
        self.assertIn("tori-mascot.png", header)
        self.assertIn("titlebar-mascot", header)
        ai_title = self.html.split('class="ai-panel-title"', 1)[1].split("</h2>", 1)[0]
        self.assertIn("lucide-bot", ai_title)
        self.assertNotIn("tori-mascot.png", ai_title)
        self.assertNotIn("lucide-house", header)
        self.assertIn("lucide-book-plus", header)
        self.assertIn("lucide-settings", header)
        self.assertNotIn("lucide-info", header)
        self.assertIn("M11 4.702", header)
        self.assertIn("m16 13 5.223", header)
        ...
        ambient_idx = header.find('id="ambientSoundButton"')
        gitsi_idx = header.find('id="gitsiPopoverButton"')
        theme_idx = header.find('id="uiThemeToggleButton"')
        notice_idx = header.find('id="headerNoticeStrip"')
        self.assertLess(home_idx, file_idx)
        self.assertLess(file_idx, admin_idx)
        self.assertLess(admin_idx, theme_idx)
        self.assertLess(theme_idx, ambient_idx)
        self.assertLess(ambient_idx, gitsi_idx)
        self.assertLess(gitsi_idx, notice_idx)
        ...
        self.assertNotIn('id="uiThemeToggleButton"', footer)
        self.assertNotIn('id="ambientSoundButton"', footer)
        self.assertNotIn('id="gitsiPopoverButton"', footer)
        self.assertIn(".app-titlebar .header-theme-btn", self.css)
        self.assertIn("border-radius: 6px", self.css.split(".app-titlebar .header-theme-btn,", 1)[1][:400])
        ...
        self.assertEqual(self.locales["ko"]["help.qa.privacy.learn.q"], "AI가 제 글을 학습하나요?")
        self.assertIn("절대 학습하지 않습니다", self.locales["ko"]["help.qa.privacy.learn.a"])
