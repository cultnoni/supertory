...
class CharacterImportAnalysisApplyTests(unittest.TestCase):
    ...
    def test_fills_empty_and_keeps_occupied(self) -> None:
        with app.database() as connection:
            ...
            self.assertEqual(seoyoon["profile_md"], "토리 인물 설정")
            self.assertEqual(seoyoon["strengths_md"], "빠른 손")
            ...
            self.assertEqual(pending[0]["analyzed_content"], "토리가 쓴 소개")
            jieun = connection.execute(
                "SELECT short_description, weaknesses_md FROM character WHERE name = '지은'"
            ).fetchone()
            self.assertEqual(jieun["short_description"], "새 인물 소개")
            self.assertEqual(jieun["weaknesses_md"], "겁이 많음")
            ...
            self.assertEqual(after, "토리가 쓴 소개")
