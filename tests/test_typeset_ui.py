...
class TypesetToolbarUiTests(unittest.TestCase):
    ...
    def test_typeset_adjust_fields_live_in_page_write(self) -> None:
        html = (ROOT / "web" / "index.html").read_text(encoding="utf-8")
        app_js = (ROOT / "web" / "app.js").read_text(encoding="utf-8")
        ...
        self.assertEqual(ko["index.쪽쓰기"], "조판")
        self.assertEqual(ko["index.쪽쓰기_열기"], "조판 열기")
        self.assertEqual(ko["index.조판"], "조판 미리보기")
        self.assertEqual(en["index.쪽쓰기"], "Typeset")
        self.assertEqual(en["index.쪽쓰기_열기"], "Open typeset")
        self.assertEqual(en["index.조판"], "Typeset preview")
        self.assertEqual(es["index.쪽쓰기"], "Composición")
        self.assertEqual(es["index.쪽쓰기_열기"], "Abrir composición")
        self.assertEqual(es["index.조판"], "Vista previa de composición")
        ...
        self.assertIn("function isPageWriteTypesetDirty", app_js)
        self.assertIn("index.쪽쓰기_조판_적용하지_않은_변경", app_js)
        self.assertIn('saveTypesetPresetFromForm', app_js.split('$("pageWriteApplyButton")', 1)[1][:400])
        close_fn = app_js.split("function closePageWrite()", 1)[1].split("function setupPageWrite()", 1)[0]
        self.assertIn("isPageWriteTypesetDirty()", close_fn)
        ...
        self.assertNotIn("repeating-linear-gradient", page_css)
        self.assertIn("background: #fbf7ef", page_css)
        self.assertIn("width: var(--page-write-viewport, 360px)", page_css)
        self.assertIn("box-sizing: border-box", page_css)
        self.assertNotIn("width: calc(var(--page-write-cols", page_css)
        self.assertIn("--page-write-viewport", app_js)
        self.assertIn("var(--page-write-cols, 16) * var(--page-write-cell, 20px)", page_css)
