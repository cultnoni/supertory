...
class CharacterDebateTests(unittest.TestCase):
    ...
    def test_ui_in_character_chat_not_helper_dropdown(self) -> None:
        ...
        self.assertIn("function toryChatCharacterAllCardHtml(", app_js)
        self.assertIn('variant: "all"', app_js)
        self.assertIn("tory-chat-character-card", app_js)
        self.assertIn("tory-chat-character-all-card", app_js)
        all_card_fn = app_js.split("function toryChatCharacterAllCardHtml(", 1)[1].split("function renderToryChatCharacterAllGrid(", 1)[0]
        self.assertNotIn("reader-persona-card", all_card_fn)
        self.assertNotIn("toryChatCharacterFullIdentity", all_card_fn)
        css = (ROOT / "web" / "styles.css").read_text(encoding="utf-8")
        self.assertIn(".tory-chat-character-card {", css)
        self.assertIn("#toryChatCharacterAllGrid .tory-chat-character-all-card", css)
        self.assertIn("overflow: hidden", css.split("#toryChatCharacterAllGrid .tory-chat-character-all-card", 1)[1].split("}", 1)[0])
        self.assertIn("max-height: 168px", css.split("#toryChatCharacterAllGrid .tory-chat-character-all-card", 1)[1].split("}", 1)[0])
