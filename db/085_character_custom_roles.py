"""Migration 85: character.role may be any short custom label (or NULL).

Keeps built-in keys (protagonist/antagonist/supporting/minor) and allows
author-defined role names such as "멘토", "히로인".
"""
...
MIGRATION_VERSION = 85
MIGRATION_NAME = "character_custom_roles"
...
CREATE TABLE character (
    ...
    role              TEXT DEFAULT NULL
                      CHECK (role IS NULL OR (length(trim(role)) BETWEEN 1 AND 40)),
...
def role_allows_custom(connection: sqlite3.Connection) -> bool:
    ...
    if not sql:
        return False
    if "role IN ('protagonist'" in sql or 'role IN ("protagonist"' in sql:
        return False
    return "length(trim(role)) BETWEEN 1 AND 40" in sql
...
def _rebuild_character_table(connection: sqlite3.Connection) -> None:
    ...
    for name in COPY_COLUMNS:
        if name not in cols:
            select_bits.append("NULL" if name == "role" else "''")
            continue
    ...
    connection.execute(_CREATE_SQL.replace("CREATE TABLE character (", "CREATE TABLE character_new_085 (", 1))
    connection.execute(
        f"INSERT INTO character_new_085 ({insert_cols}) SELECT {select_sql} FROM character"
    )
    connection.execute(
        "UPDATE character_new_085 SET role = NULL "
        "WHERE role IS NOT NULL AND (length(trim(role)) < 1 OR length(trim(role)) > 40)"
    )
    connection.execute("DROP TABLE character")
    connection.execute("ALTER TABLE character_new_085 RENAME TO character")
...
def apply(connection: sqlite3.Connection) -> None:
    ...
    if MIGRATION_VERSION in applied and role_allows_custom(connection):
        return
    if not role_allows_custom(connection):
